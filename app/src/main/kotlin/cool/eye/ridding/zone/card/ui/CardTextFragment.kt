...
class CardTextFragment : BaseFragment() {
    ...
    fun loadText() {
        recyclerview.adapter = CardAdapter(DBHelper.getCardInfo(context))
    }
        ...
        override fun onBindViewHolder(holder: CardHolder, position: Int) {
            ...
            holder.view.setOnClickListener {
                AlertDialog.Builder(context)
                        .setMessage("你要做什么?")
                        .setNegativeButton("删除", { dialog, i ->
                            DBHelper.removeCardInfo(context, infos[position])
                            loadText()
                        })
                        .setNeutralButton("复制文本", { dialog, i ->
                            copy(infos[position])
                        })
                        .setPositiveButton("确定", { dialog, witch ->
                            CardHelper.shareMsg(context, infos[position])
                        }).show()
                false
            }
        }
